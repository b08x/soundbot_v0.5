...
live_loop :pulsar, sync: :pulse, auto_cue: false do
  ...
  with_fx :reverb, mix: 0.3, room: 0.3, amp: 0.75 do
    notes.size.times do
...
live_loop :drone, auto_cue: false do
  ...
  scl = scale(:c5, :harmonic_major, num_octaves: 1)[0..4]
...
scales_arr = []
2.times do
  scl = scale([:c5, :c6].choose, :harmonic_major, num_octaves: 2)
...
comment do
  live_loop :static, sync: :stc, auto_cue: false do
    ...
    scl = scale(:c2, :harmonic_major, num_octaves: 2).choose
